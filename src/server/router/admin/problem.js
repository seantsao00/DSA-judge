...
async function updateProblemByGzip(id, file) {
  ...
  try {
    ...
    logger.info(`extract ${file.path} to ${destDir}`);
    // extract the archive to destination directory
    await tar.extract({ gzip: true, cwd: destDir, file: file.path, strip: 1 });
    logger.info(`extract ${file.path} to ${destDir} successfully`);
  } finally {
    fs.unlink(file.path, (err) => {
      if (err) logger.error(`unlink ${file.path} failed with error ${err}`);
    });
  }
}
...
router.put('/',
  ...
  wrap(async (req, res) => {
    ...
    try {
      ...
      await problem.save();
      logger.info(`Successfully create problem #${id}`);
    } catch (e) {
      logger.error(e);
